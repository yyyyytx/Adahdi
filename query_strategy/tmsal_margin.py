...
class TMSMarginSampling(BaseStrategy):
    ...
    def query(self, n_select):
        ...
        self.center_sim_thrs = self.trainer.get_center_sim_thr()

        centroids = self.trainer.center_loss.centers


        loader_list = self.build_divided_unlabel_loader(512)
        delta = []
        for i, unlabel_loader in enumerate(loader_list):
            print('unlabel loader: %d/%d' % (i + 1, len(loader_list)))
            tmp_delta = self.cal_domain_margin(unlabel_loader, centroids)
