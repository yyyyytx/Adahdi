...
class DANNTrainer(BaseTrainer):
    ...
    def train(self, name, n_epoch=None):
        ...
        domain_opti = torch.optim.Adadelta(self.domain_discriminator.parameters(), lr=1.0)
        ...
        for epoch in tqdm(range(self.train_cfg.epochs)):
            accTrain, lossTrain = self.combine_train_each_epoch([opti, domain_opti], [train_loader, unlabeled_loader], epoch, name)
