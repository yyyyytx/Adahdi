...
class CDALSampling(BaseStrategy):
    ...
    def query(self, n_select):
        ...
        self.num_episode = 5
        self.classes = self.net.n_label
        self.hidden_dim = self.net.get_embedding_dim()
        self.beta = 0.01
        ...
        probs, embeddings, _, _ = self.predict_probs_and_embed(unlabel_loader)
        # probs_l, _, _, _ = self.predict_probs_and_embed(label_loader)

        # probs, embeddings = self.predict_prob_embed(self.X[idxs_unlabeled], self.Y[idxs_unlabeled])
        chosen = self.train_cdal(str(self.query_count), F.softmax(probs, dim=1).numpy(), n_select)

        return unlabel_ind[chosen]

        # tmp_ind = self.select_coreset(F.softmax(probs, dim=1).numpy(), F.softmax(probs_l, dim=1).numpy(), n_select)
    ...
    def train_cdal(self, name, features, number_of_picks):
        model = DSN(in_dim=self.net.n_label).cuda()
