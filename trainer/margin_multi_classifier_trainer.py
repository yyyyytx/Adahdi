...
class MarginMultiClassifierTrainer(BaseTrainer):
    ...
    def __init__(self, net, train_cfg, label_info, train_ds, select_ds, test_ds, writer, strategy_cfg=None, is_amp=False, logger=None):
        ...
        # print(self.multi_classifier.parameters_list())

    def train(self, name, n_epoch=None):
        self.center_loss = MultiCenterLoss(self.net.n_label, self.net.get_embedding_dim(), self.train_cfg.dataset_number)
        ...
        # multi_opti = torch.optim.AdamW(self.multi_classifier.parameters_list(), lr=0.1)
        multi_opti, multi_sched = self.build_multi_optimizer(self.multi_classifier)
        # multi_classifier_opti = torch.optim.Adadelta(self.multi_classifier.parameters_list(0.1), lr=0.1)
        ...
        for epoch in tqdm(range(self.train_cfg.epochs)):
            ...
            if epoch+1 == self.train_cfg.center_epoch:
                ...
                # print('centers nan:', torch.sum(self.center_loss.centers == torch.nan))
                # print(torch.sum(centers != self.center_loss.centers))
                # print(centers[centers != self.center_loss.centers])
                # print(self.center_loss.centers[centers != self.center_loss.centers])
                # print('centers nan:', torch.sum(self.center_loss.centers == torch.nan))

                assert torch.sum(centers != self.center_loss.centers) == 0
        ...
        for epoch in tqdm(range(self.train_cfg.epochs)):
            ...
            if (epoch + 1) % self.train_cfg.val_interval == 0:
                current_acc = self.multi_classifier_test()['current']
                cur_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
                str = cur_time + 'multi classifier train epoch: %d  train acc %.4f cur acc %.4f loss %.4f' % (
        ...
        for Xs, ys, ind, ds_ind in train_loader:
            ...
            with torch.enable_grad():
                for i in range(self.train_cfg.dataset_number):
                    ...
                    margin_loss += ds_margin_loss
                    correct = torch.sum(torch.max(y_hats, 1)[1] == ds_ys)
                    total_correct += correct
            # loss_multi = self.multi_classifier
            total_loss = (classifier_loss + margin_loss)/self.train_cfg.dataset_number
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
            tot_loss += total_loss
    ...
    def combine_train_each_epoch(self, optimizer, train_loader, epoch, name):
        ...
        for Xs, ys, ind, ds_ind in train_loader:
            ...
            with torch.enable_grad():
                ...
                margin_loss = Margin_loss(F.normalize(y_hats, dim=1), ys)
