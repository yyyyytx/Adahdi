...
class RecallMultiClassifierTrainer(BaseTrainer):
    ...
    # def multi_classifier_train_each_epoch(self, optimizer, multi_classifier_mask, l_norm_feats, l_true_labels):
    #     # print(l_norm_feats.shape, l_d)
    #     self.net.train()
    #     self.multi_classifier.train()
    #     total_correct = 0
    #     for i in range(len(self.multi_classifier.multi_classifiers)):
    #         print('train classifier ', i)
    #         for epoch in tqdm(range(self.train_cfg.multi_epochs)):
    #             ds_train_mask = multi_classifier_mask[i]
    #             d = torch.tensor(range(torch.sum(ds_train_mask)))
    #             sampler = RandomSampler(data_source=d)
    #             batch_sampler = BatchSampler(sampler, batch_size=self.train_cfg.train_bs, drop_last=False)
    #             for batch_idx in batch_sampler:
    #                 feats = l_norm_feats[ds_train_mask][batch_idx]
    #                 ds_ys = l_true_labels[ds_train_mask][batch_idx]
    #                 with torch.enable_grad():
    #                     y_hats = self.multi_classifier(feats, i)
    #                     ds_classifier_loss = F.cross_entropy(y_hats, ds_ys)
    #
    #                     correct = torch.sum(torch.max(y_hats, 1)[1] == ds_ys)
    #                     total_correct += correct
